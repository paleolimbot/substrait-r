...
``` r
substrait_project(.compiler, ..., .drop_columns = character())

substrait_select(.compiler, ...)
```
...
- .drop_columns:
  ...
  A character vector of columns to explicitly drop before adding
  expressions specified in `...`.
